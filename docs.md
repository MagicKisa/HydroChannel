...
## tare_weight.py - скрипт для перевода показаний тензодатчика в силу
...
## Correct_speed.py - служит для перевода показаний скоростемера в скорость
...
## laser_div.py - скрипт для перемещения показаний двух дальномеров из одного файла в два отдельных 
...
При смене номеров лазеров. Нужно заменить в коде скрипта laser_nose='26194' на laser_nose=номер нового лазера на носу корабля

laser_nose='26193' на laser_nose=номер нового лазера на корме корабля

## interpolate.py - сводит несколько таблиц в одну
...
a_res.csv, a+1_res.csv ... b_res.csv

> [!NOTE]
> если указать a = b, то посчитается res файл a_res.csv
...
## inversie_csv.py - обращает координату и время
...
## relate_laser.py - вычитает из одних показаний лазера другие по координате
...
Для 1-го эксперимента с входными данными
...
Рельсы по которым едет телега не являются строго горизонтальными. Относительные показания позволяют не учитывать изменение показаний лазеров связанных с этим.

## Read_ser.py - чтение данных из портов и сохранение в csv

### Назначение 

Чтение данных полученных с датчиков ардуино по usb портам

### Параметры

`python3 Read_ser.py port1 port2 ... portn num_of_experiment`

Последний переданный параметр - номер эксперимента

Параметры до него - номера портов для считывания.

### Применение

Для i-го эксперимента

`python3 Read_ser.py 0 1 i`

где параметры

0 - номер порта тензодатчика

1 - номер порта скоростемера

i - номер эксперимента

Для завершения считывания и сохранения в файл нужно нажать x

### Результат

Для работы команды сверху

i_0.csv - файл с показаниями тензодатчика

i_1.csv - файл с показаниями датчика скорости

### Предупреждения

Для смены системы нужно зайти в код и поменять переменную system='Linux' на system='Windows'

Номера портов - целые числа от 0 до n.

Для того чтобы определить номер порта можно зайти в приложение arduino ide -> tools -> port

Датчик силы называется arduino uno, датчик скорости - arduino leonardo

> [!WARNING]
> Если новая версия скрипта не работает, можно достать 100% рабочую старую версию из папки Старая версия скрипта

## conv.py - применяет к данным все необходимые скрипты с параметрами по умолчанию

### Назначение

обрабатывает файлы экспериментов, и получает сводные таблицы относительно нулевых показаний лазеров 
...
В папке должны находиться файлы 10 проходов и прохода назад

1_0.csv, 1_1.csv, 1.csv ..., 10_0, 10_1.csv, 10.csv, back_0.csv, back_1.csv, back.csv

Если экспериментов будет больше или меньше - скрипт работать не должен.
...
### Прошивка
sensor_10Hz.ino - для считывания показаний скоростемера и передачи на компьютер

Установить с помощью arduino ide прошивку на плату к которой подключен скоростемер

### Подключение

Подключить плату по usb к компьютеру

## Uno
### Прошивка

tenzo_sk.ino -  для считывания показаний тензодатчика и передачи на компьютер

Необходимо скачать библиотеки <GyverHX711.h> и "DHT.h" - для этого можно воспользоваться arduino ide

Установить с помощью arduino ide прошивку на плату к которой подключен тензодатчик.

### Подключение
![image](https://github.com/MagicKisa/HydroChannel/assets/105859497/a5101e16-d091-45ff-99da-f5d85abe1419)

Подключить плату по usb к компьютеру

## Считывание данных

для считывания использовать Read_ser.py
